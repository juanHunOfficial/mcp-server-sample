...
def get_code_reviewers() -> ReviewerList | str:
    """Retrieve the code reviewers for the current project"""
    try:
        # Connect to the database
        conn = sqlite3.connect("./data/code_reviewers.db")
        cursor = conn.cursor()

        # Execute query to fetch the names of all code reviewers
        cursor.execute("SELECT name FROM code_reviewers;")
        
        # Fetch the result
        result = cursor.fetchall()

        return ReviewerList(reviewers=result)
    except sqlite3.Error as e:
        return f"Error with SQLite3 Database connection or query: {e}"
